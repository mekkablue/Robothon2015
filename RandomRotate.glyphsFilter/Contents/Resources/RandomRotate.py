...
class RandomRotate ( GSFilterPlugin ):
	"""
	...
	"""
	maxRotationField = objc.IBOutlet()
	...
	def init( self ):
		...
		"""
		try:
			NSBundle.loadNibNamed_owner_( "RandomRotateDialog", self )
			random.seed()
			return self
	...
	def setup( self ):
		...
		try:
			...
			self.maxRotation = self.setDefaultFloatValue( "maxRotation", 15.0, FontMaster )
			self.maxRotationField.setFloatValue_( self.maxRotation )
	...
	def setMaxRotate_( self, sender ):
		...
		"""
		try:
			maxRotation = sender.floatValue()
			if maxRotation != self.maxRotation:
				self.maxRotation = maxRotation
				self.process_( None )
		except Exception as e:
			self.logToConsole( "setMaxRotate_: %s" % str(e) )
	
	def randomAngle( self, maxAngle ):
		try:
			angle = random.random() * 2 * maxAngle - maxAngle
			return angle
		except Exception as e:
			self.logToConsole( "randomAngle: %s" % str(e) )
	...
	def processLayerWithValues( self, Layer, maxRotation ):
		"""
		...
		"""
		try:
			angle = self.randomAngle( maxRotation )
			center = self.centerOfLayer( Layer )
			rotation = self.rotationTransform( angle, center )
			Layer.transform_checkForSelection_doComponents_( rotation, False, True )
		except Exception as e:
		...
		try:
			...
			if len( Arguments ) > 1:
				...
				if not "clude:" in Arguments[1]:
					maxRotation = Arguments[1].floatValue()
			...
			for Glyph in glyphList:
				...
				self.processLayerWithValues( Layer, maxRotation ) # add your class variables here
		except Exception as e:
	...
	def process_( self, sender ):
		...
		try:
			...
			for k in range(len( ShadowLayers )):
				...
				self.processLayerWithValues( Layer, self.maxRotation ) # add your class variables here
			Layer.clearSelection()
		
			# Safe the values in the FontMaster. But could be saved in UserDefaults, too.
			FontMaster = self.valueForKey_( "fontMaster" )
			FontMaster.userData[ "maxRotation" ] = NSNumber.numberWithInteger_( self.maxRotation )
			
			# call the superclass to trigger the immediate redraw:
			super( RandomRotate, self ).process_( sender )
		except Exception as e:
